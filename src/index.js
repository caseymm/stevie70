...
d3.select(window).on('scroll', d => {
  ...
  if(fLeg['y'] <= 0.5 && Math.abs(fLeg['y']) <= fSect['height'] && (fSect['y'] - fLeg['height']*2) < 0 && Math.abs(fSect['y']) <= fSect['height']){
  ...
  if(sLeg['y'] <= 0.5 && Math.abs(sLeg['y']) <= sSect['height'] && (sSect['y'] - sLeg['height']*2) < 0 && Math.abs(sSect['y']) <= sSect['height']){
